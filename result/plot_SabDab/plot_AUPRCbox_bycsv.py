...
def plot_boxplot_from_csv(csv_path, save_path=None, col_order=None):
    # 从 CSV 文件读取数据
    ...
    # 绘制箱线图
    box_plot = sns.boxplot(x=labels, y=data_for_plot, showfliers=True, palette='pastel', width=0.5)
    ...
    plt.ylabel('AUPRC', fontsize=28, labelpad=20)
...
# 指定 CSV 文件路径
# 画SabDab的AUC-ROC箱式图
csv_path = '/data/gm_data/AbAtInteraction/AbAtIPA/abatInter_SCA/SabDab/trained_models/res_dir/AUC-PR.csv'
# 画AUC-PR的箱式图
csv_dir = osp.dirname(csv_path)
fig_path = osp.join(csv_dir, basename(csv_path).split('.')[0] + '.tif')
# col_order=['IPA-Paddle', 'IPA-twins', 'GCN-paddle', 'GCN-twins','CNN-CKSAP']
# 调用函数画箱式图
