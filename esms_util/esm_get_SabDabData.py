...
from esms_util.esm_inter_utils import get_pos_data
...
from training.cluster_db import clustrByAtseq, family_with_subset
from training.cdhit_Ab import get_tree_TaxonListCLS
...
import random
data_dir = osp.join(project_dir, 'SabDab', 'SabDabdatabase')
SabDab_dir = osp.dirname(data_dir)
ddp = DataDirPath()
random.seed(22)
np.random.seed(22)
...
def get_AtAbDataDict(info_dict, embeder, reci=None, atseq=True):
    ...
    for AbAt_ids in tqdm(info_dict.keys()):
        ...
        for abatch in info_dict[AbAt_ids]:
            ...
            # if '6p3s' not in AbAtId_string:  continue  # debug
...
def get_data(embeder, non_redun='Atseq', atcluster_trunc=0.90, negRatioPos=1, dataSetName='SabDab', poskey='sudo_at', kfold=5):
    ...
    train_fold_subset = fold_abatid2suset(train_folds, abatid_affine_subsetid)
    val_fold_subset = fold_abatid2suset(valid_folds, abatid_affine_subsetid)
...
def fold_abatid2suset(fold_dict, abatid_affine_subsetid):
    fold_subset_dict = {}
    for k, v in fold_dict.items():
        subset_ls = sorted(list(set([abatid_affine_subsetid[ss] for ss in v])))
        fold_subset_dict[k] = subset_ls
    return fold_subset_dict
...
def construct_trainval_df(pos_df, train_folds, valid_folds, subset_contain_abatid, abatid_affine_subsetid, family_subset_info, family_contain_abatid, abatid_affine_fammid, negRatioPos=1, poskey='sudo_at'):
    ...
    for k_i in range(kfold):
        ...
        train_neg_df = negdf_by_nosamesubset(pos_df, train_pos_df, subset_contain_abatid, abatid_affine_subsetid, family_subset_info, family_contain_abatid, abatid_affine_fammid, negRatioPos, poskey)
        valid_neg_df = negdf_by_nosamesubset(pos_df, valid_pos_df, subset_contain_abatid, abatid_affine_subsetid, family_subset_info, family_contain_abatid, abatid_affine_fammid, negRatioPos, poskey)
...
def negdf_by_nosamesubset(total_posdf, subpos_df, subset_contain_abatid, abatid_affine_subsetid, family_subset_info, family_contain_abatid, abatid_affine_fammid, negRatioPos=1, poskey='sudo_ab'):
    """负样本来自不同的subset, 且可以来自本家族内部也已来自其他家族"""
    negdf = pd.DataFrame(columns=subpos_df.columns)
    # neg_keepcols = ['sudo_at', 'At_name', 'Atseq', 'init_at_mergeobj', 'init_at_mergeobj', 'at_mergeobj', 'at_preDir']  # 这里错了, 现在应该反过来 固定了At, 而且原本sudo_at也设置错了
    # total_At_abatids = total_posdf['sudo_at'].to_list()
    neg_changeAb_keepcols = ['sudo_ab', 'merged_index', 'Ab_name', 'Hseq', 'Lseq', 'Abseq', 'len_0', 'len_1', 'cdr_fr_info', 'init_ab_mergeobj', 'ab_mergeobj', 'ab_preDir']
    ...
    for pos_abatid in tqdm(pos_abatids):
        ...
        # print(f'{pos_abatid} not in family cluster')
        now_fold_abatids = subpos_df['sudo_at'].to_list()
        candi_neg_abatids = sorted(list(set(now_fold_abatids) - set(subset_contain_abatid[abatid_affine_subsetid[pos_abatid]])))
        pos_dfi = subpos_df[subpos_df[poskey] == pos_abatid]  # pos中是对抗原进行固定的

        neg_abatidnum = pos_dfi.shape[0] * negRatioPos
        if neg_abatidnum > len(candi_neg_abatids):
            neg_abatids = np.random.choice(candi_neg_abatids, size=neg_abatidnum, replace=True)
        else:
            neg_abatids = random.sample(candi_neg_abatids, neg_abatidnum)
        neg_dfi = pd.concat([pos_dfi] * negRatioPos, ignore_index=True).reset_index(drop=True)
        for j_, neg_abatid in enumerate(neg_abatids):
