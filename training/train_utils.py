...
from utils.geometry import dist, angle, dihedral


def inter_class_loss(pred, label, criterion, loss_weight=1.0):
    label = label.view(-1)
    # if label.ndim > 1:
    #     label = label.squeeze()
    # elif label.ndim == 0:
    #     label = label.view(1)
    return criterion(pred, label) * loss_weight
...
def kabsch(
...
):  # 点云配准算法（对于点云P和点云P'，如何旋转和平移点云P使其尽量贴合点云P'）
    X = rearrange(
        mobile,
        "... l d -> ... d l",
    )  # mobile 是实际骨架坐标tensor
    Y = rearrange(
        stationary,
        "... l d -> ... d l",
    )

    #  center X and Y to the origin 找到骨架所有原子的坐标中心
    ...
    # Optimal rotation matrix via SVD 通过SVD优化旋转矩阵
...
def do_kabsch(
    ...
    stationary,
    align_mask=None,
):  # stationary是pred的coords，mobile是实际的coord_label
...
def kabsch_mse(
    ...
    )  # 优化得到旋转矩阵，将预测结构与已有结构进行对齐
    mse = F.mse_loss(
        pred,
        aligned_target,
        reduction='none',
    ).mean(-1)  # mse=1/n*sum((y_i-y'_i)**2)
...
def bond_length_l1(
    ...
    mask,
    offsets=[1, 2],
):  # bond_loss 只考虑每个氨基酸的三个重原子N, Cα, C
    losses = []
    for c in range(pred.shape[0]):
        ...
        for o in offsets:
            ...
            losses.append(torch.abs(pred_lens[m_] - target_lens[m_], ).mean() / o)  # bond_loss中是L1损失
