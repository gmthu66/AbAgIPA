...
def quaternion_to_matrix(quaternions: torch.Tensor) -> torch.Tensor:
    ...
    """
    r, i, j, k = torch.unbind(quaternions, -1)  # [b, r, 4]
    two_s = 2.0 / (quaternions * quaternions).sum(-1)  # 2 / （第i个四元数的各元素平方和）
    ...
    return o.reshape(quaternions.shape[:-1] + (3, 3))  # reshape成为 [b, r, 3, 3] 这样的旋转矩阵
...
def quaternion_multiply(a: torch.Tensor, b: torch.Tensor) -> torch.Tensor:
    ...
    """
    ab = quaternion_raw_multiply(a, b)  # 作用是对两个用四元数形式表示的旋转矩阵进行矩阵乘法
