...
from SabDab.modelSab.IPA_paddleWithPoint import IPA_PaddleWithPoint
...
def init_config(config):
    ...
    config.par_out_dir = osp.join(spect_AtDir, f'trained_models/IPA_PaddleWithPointTrueLable_{config.embed_type}')
...
def init_from_ckpt(model_ckpt, user_config=None):
    ...
    Model = FAbAtInter if user_config.embed_type is 'Bert' else IPA_PaddleWithPoint
...
def train(model_ckpt=None, try_gpu=True, id_fold=None):
    ...
    """TrueLabel修改之处"""
    for abatid in AbAtPair_dict:
        true_coords = AbAtPair_dict[abatid]['Ab_data']['label_coords']
        AbAtPair_dict[abatid]['Ab_data']['pred_coords'] = true_coords.unsqueeze(0)  # 将true_coords替换pred_coords
