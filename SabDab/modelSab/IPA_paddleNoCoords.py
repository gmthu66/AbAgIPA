...
from model.components import TriangleGraphTransformer, IPAEncoder, IPATransformer, Inter_GlobalMax, BinaryClassifier, IPAEncoder_ForInter_NoCoords
...
class IPA_PaddleNoCoords(pl.LightningModule):
    ...
    def __init__(
        ...
        # 将template的(rotation, translation)、目标序列的pair_rep(边信息)Encode到目标序列的node_feat中，并输出更新后的node_feat
        self.Ab_template_ipa = IPAEncoder_ForInter_NoCoords(
            dim=self.node_dim,
        ...
        self.At_template_ipa = IPAEncoder_ForInter_NoCoords(
            dim=self.node_dim,
            depth=self.At_temp_ipa_depth,
            heads=self.temp_ipa_heads,
            # require_pairwise_repr=True,
