...
from model.components import TriangleGraphTransformer, IPAEncoder, IPATransformer, Inter_OnlyAbGlobalMean, BinaryClassifier, IPAEncoder_ForInter
...
class FAbAtInterPhysic(pl.LightningModule):
    ...
    def __init__(
        ...
        self.inter_layer = Inter_OnlyAbGlobalMean()
        self.inter_class = BinaryClassifier(self.node_dim, self.node_dim * 2, dropout_rate=0.5)
    ...
    def forward(
        ...
        inter_feat = self.inter_layer(ab_str_nodes, ab_mask=Ab_cdr_mask)  # 只是在这里进行cdr_mask
