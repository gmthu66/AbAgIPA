"""无三角图网络, 不使用edge feature, 基于node feature和Ti的affinity进行更新, 抗体和抗原不采用同一个IPA, pooling采用maxpooling"""
...
from model.components import TriangleGraphTransformer, IPAEncoder, IPATransformer, Inter_GlobalMax, BinaryClassifier, IPAEncoder_ForInter
...
class IPA_Paddle(pl.LightningModule):
    ...
    def __init__(
        ...
        self.return_attn = config.return_attn if 'return_attn' in dir(config) else False
        ...
        # self.esm_attn_dim = 22
        if self.node_dim < 36:
            self.esm_feat_dim = int(self.node_dim / 2)
            self.esm_attn_dim = 36
        else:
        ...
        # 将template的(rotation, translation)、目标序列的pair_rep(边信息)Encode到目标序列的node_feat中，并输出更新后的node_feat
        self.Ab_template_ipa = IPAEncoder_ForInter(
            dim=self.node_dim,
            depth=self.At_temp_ipa_depth,
            heads=self.temp_ipa_heads,
            # require_pairwise_repr=True,
            require_pairwise_repr=False,  # no_trainglar 没有三角注意力时的网络设置
            return_attn = self.return_attn,
        )
        self.At_template_ipa = IPAEncoder_ForInter(
            dim=self.node_dim,
            depth=self.At_temp_ipa_depth,
            heads=self.temp_ipa_heads,
            # require_pairwise_repr=True,
        ...
        self.inter_layer = Inter_GlobalMax()
    ...
    def forward(
        ...
        Ab_VHLlens, max_abseqlen, batch_size = data.Ab_VHLlen, data.Ab_maxlen, data.Ab_phisicAttention.size(0)
        ...
        if Ab_temp_mask is not None:  res_temp_mask = rearrange(Ab_temp_mask, "b (l a) -> b l a", a=4,).all(-1).to(self.device)
        ...
        if not self.predcoords_exist:
            ...
            if data.Ab_mut_range is not None:
                Ab_pred_coords = data.Ab_coords_label
            else:
                Ab_pred_coords = data.Ab_pred_coords
            flat_coordsi = rearrange(Ab_pred_coords[:, :, :4], "b l a d -> b (l a) d",)

        # ------------------------------------------------- 相互作用模块 --------------------------------------------
        if self.esm_feat_dim == 11:
            At_embeddings = At_embeddings[:, :, 25:]
            Ab_phisicEmbed = Ab_phisicEmbed[:, :, 25:]
            At_attentions = torch.cat([At_attentions[:, :, :, 25:36], At_attentions[:, :, :, -11:]], dim=-1)
            Ab_phisicAttention = torch.cat([Ab_phisicAttention[:, :, :, 25:36], Ab_phisicAttention[:, :, :, -11:]], dim=-1)
        else:
            At_embeddings = At_embeddings[:, :, :]
            Ab_phisicEmbed = Ab_phisicEmbed[:, :, :]
            At_attentions = torch.cat([At_attentions[:, :, :, :], At_attentions[:, :, :, :]], dim=-1)
            Ab_phisicAttention = torch.cat([Ab_phisicAttention[:, :, :, :], Ab_phisicAttention[:, :, :, :]], dim=-1)
        at_str_nodes = self.ab_node_transform(At_embeddings)
        at_str_edges, ab_str_edges = None, None
        # at_str_edges = self.ab_edge_transform(At_attentions)
        # at_str_nodes, at_str_edges = self.At_main_block(at_str_nodes, at_str_edges, mask=At_res_batch_mask)

        ab_str_nodes = self.ab_node_transform(Ab_phisicEmbed)
        # ab_str_edges = self.ab_edge_transform(Ab_phisicAttention)
        # ab_str_nodes, ab_str_edges = self.At_main_block(ab_str_nodes, ab_str_edges, mask=Ab_res_batch_mask)
        ...
        if self.only_muterange and exists(data.Ab_mut_range):  # 是否只是保存突变区域的特征, 主要针对HER2的CDRH3
            ...
            Ab_dumy_translations, Ab_dumy_rotations = self.get_coords_tran_rot(flat_coordsi, batch_size, ab_str_nodes.size(1),)
        if not self.return_attn:
            at_str_nodes = self.Ab_template_ipa(at_str_nodes, translations=At_truth_translations, rotations=At_truth_rotations, pairwise_repr=None, mask=At_res_batch_mask)
            ab_str_nodes = self.At_template_ipa(ab_str_nodes, translations=Ab_dumy_translations, rotations=Ab_dumy_rotations, pairwise_repr=None, mask=Ab_res_batch_mask)
        else:
            at_str_nodes, Atres_attn, Atpoint_attn = self.Ab_template_ipa(at_str_nodes, translations=At_truth_translations, rotations=At_truth_rotations, pairwise_repr=None, mask=At_res_batch_mask)
            ab_str_nodes, Abres_attn, Abpoint_attn = self.At_template_ipa(ab_str_nodes, translations=Ab_dumy_translations, rotations=Ab_dumy_rotations, pairwise_repr=None, mask=Ab_res_batch_mask)
        ...
        if exists(Ab_coords_label) and (not self.predcoords_exist):
        ...
        bert_embs = bert_feats if (return_embeddings and not self.predcoords_exist) else None
        bert_attn = bert_attn if (return_embeddings and not self.predcoords_exist) else None
        gt_embs = gt_embs if (return_embeddings and not self.predcoords_exist) else None
        structure_embs = structure_embs if (return_embeddings and not self.predcoords_exist) else None
        output = InterFoldOutput(
            ...
            Atres_attn=Atres_attn,
            Abres_attn=Abres_attn,
            Atpoint_attn=Atpoint_attn,
            Abpoint_attn=Abpoint_attn,
        )
