...
from model.components import TriangleGraphTransformer, IPAEncoder, IPATransformer, Inter_GlobalMean, BinaryClassifier, IPAEncoder_ForInter
...
class FAbAtInterPhysic(pl.LightningModule):
    ...
    def forward(
        ...
        if not self.return_attn:
            ...
            at_str_nodes, res_attn, point_attn = self.At_template_ipa(at_str_nodes, translations=At_truth_translations, rotations=At_truth_rotations, pairwise_repr=at_str_edges, mask=At_res_batch_mask)
            ab_str_nodes, res_attn, point_attn = self.At_template_ipa(ab_str_nodes, translations=Ab_dumy_translations, rotations=Ab_dumy_rotations, pairwise_repr=ab_str_edges, mask=Ab_res_batch_mask)
