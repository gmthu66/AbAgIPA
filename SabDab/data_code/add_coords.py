...
def add_coords(model_ckpt=None, try_gpu=True, id_fold=None):
    ...
    model.eval()
    pre_embed_dir = osp.join(spect_AtDir, 'SabDabdatabase')
    with torch.no_grad():
        Ab_abatid_excusedls = []
        Ab_abatids = [dpath for dpath in os.listdir(pre_embed_dir) if check_AbsDir_indpath(osp.join(pre_embed_dir, dpath))]
        for Ab_abatid in Ab_abatids:
            # At_abatid = Ab_abatid
            Ab_data = AbAtPair_dict[Ab_abatid]['Ab_data']
            if len(''.join([seq for seq in Ab_data['seq_dict'].values()])) > 350:
                print('Ab HL aaseq too long:', Ab_abatid)
                Ab_data = modify_Abdata(Ab_data, osp.join(pre_embed_dir, Ab_abatid))
            ...
            Ab_abatid_excusedls.append(Ab_abatid)
            # torch.load(predAb_path)
            print(f'{Ab_abatid} end')

        # for indexi in tqdm(range(data_df.shape[0])):
        #     data_si = data_df.iloc[indexi]
        #     pos_index = int(data_si['pos_dataid'])
        #     Ab_abatid, At_abatid = data_si['sudo_ab'], data_si['sudo_at']  # 在获取负样本数据的时候错误, sudo_at的id并不是真实的id

        #     if Ab_abatid not in Ab_abatid_excusedls:
        #         # Ab_abatid_excusedls.append(Ab_abatid)
        #         # continue  # debug
        #         Ab_data = AbAtPair_dict[Ab_abatid]['Ab_data']
        #         assert compare_h_l_values(data_si, Ab_data['seq_dict'])
        #         batch_Ab_data = process_Abdata(Ab_data, db_args=embeder.database_args, tmp_flg=False, mutate_range=None)
        #         At_data = AbAtPair_dict[At_abatid]['At_data']
        ...
        # for indexi in tqdm(range(data_df.shape[0])):
        #     # if indexi < 1514:  continue  # debug
        #     data_si = data_df.iloc[indexi]
        #     At_abatid, Ab_abatid = data_si['sudo_ab'], data_si['sudo_at']  # 在获取负样本数据的时候错误, sudo_at的id并不是真实的id
