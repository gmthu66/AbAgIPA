...
from model.components import TriangleGraphTransformer, IPAEncoder, IPATransformer, Inter_GlobalMean, BinaryClassifier
...
class FAbAtInterPhysic(pl.LightningModule):
    ...
    def forward(
        ...
        ab_nodes_mat = self.ab_node_transform(Ab_phisicEmbed)
        ab_edges_mat = self.ab_edge_transform(Ab_phisicAttention)
        ...
        if self.only_muterange and exists(data.Ab_mut_range):  # 是否只是保存突变区域的特征, 主要针对HER2的CDRH3
            st, ed = (data.Ab_mut_range[0], data.Ab_mut_range[1]) if (res_temp_mask.size(0) == 1) else (data.Ab_mut_range[0][0], data.Ab_mut_range[0][1])
            Ab_dumy_translations, Ab_dumy_rotations = self.get_coords_tran_rot(flat_coordsi[:, (st - 1) * 4 : (ed + 2) * 4, :], batch_size, ab_nodes_mat.size(1),)
            Ab_batch_mask[:, (st - 1) * 4 : (ed + 2) * 4,] = False  #若只是输入突变区域,则对Ab_batch_mask进行更改, 那结构预测loss计算时会将Ab的cdr区域mask掉
        else:
            Ab_dumy_translations, Ab_dumy_rotations = self.get_coords_tran_rot(flat_coordsi, batch_size, ab_nodes_mat.size(1),)
