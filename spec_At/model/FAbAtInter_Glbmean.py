...
class FAbAtInter(pl.LightningModule):
    ...
    def forward(
        ...
        inter_loss_weight = 1.0 if not exists(Ab_coords_label) else 10.0
        ...
        # ab_str_nodes = torch.index_select(str_nodes, dim=1, index=Ab_cdr_indexs) if Ab_cdr_indexs is not None else str_nodes
        # Ab_res_batch_mask = torch.index_select(res_batch_mask, dim=1, index=Ab_cdr_indexs) if Ab_cdr_indexs is not None else res_batch_mask
        ...
        if exists(data.Ab_mut_range):  Ab_batch_mask[:, (data.Ab_mut_range[0] - 1) * 4 : (data.Ab_mut_range[1] + 2) * 4,] = False
        if exists(Ab_coords_label):
            ...
            for sl_i, sl in enumerate(seq_lensi):
                ...
                if sl == 0 or align_mask_.sum() == 0 or res_batch_mask_.sum() == 0:
                    continue
                if exists(data.Ab_mut_range):  res_batch_mask_[:, (data.Ab_mut_range[0] - 1) : (data.Ab_mut_range[1] + 2),] = False
