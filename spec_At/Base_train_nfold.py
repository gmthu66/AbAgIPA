...
class TrainerBase(ABC):
    ...
    def __init__(self, config, data, model):
        ...
        self.device, self.num_data_workers = config.device, config.num_data_workers
        ...
        self.train_val_testflg = getattr(data, 'train_val_testflg') if hasattr(data, 'train_val_testflg') else False
        self.train_loader = data.train_loader
        self.train_sampler = data.train_sampler
        if self.train_val_testflg:  self.valid_loader = data.valid_loader
        ...
        with tqdm(range(self.start_epoch, self.epochs+1)) as tq:
            for epoch in tq:
                ...
                if epoch % self.test_freq == 0:
                    ...
                    if not self.train_val_testflg:
                        test_loss, test_metric_dict = valid_loss, valid_metric_dict
                    else:
    ...
    def _synergy_optimizer(self, config):
        ...
        if len(finetune_pnames) < 1:
            optimizer = optim([fix_dict, inter_dict])
        else:
...
def substrlsInString(string, substrLs):
    for substr in substrLs:  # 容易全都变成了fineturn层
