...
class TrainerBase(ABC):
    ...
    def __init__(self, config, data, model):
        ...
        time.sleep(3)
        self.model = model
        self.synergy_flg = config.synergy if 'synergy' in dir(config) else False
        self.train_type = 'TrainValTest' if 'loader_type' not in dir(data) else data.loader_type
        if hasattr(data, 'train_val_testflg'):
            self.train_type = 'TrainValTest' if (getattr(data, 'train_val_testflg') is True) else data.loader_type
        if self.synergy_flg:
        ...
        if self.train_type is not 'Nfold':
    ...
    def train(self):
        ...
        with tqdm(range(self.start_epoch, self.epochs+1)) as tq:
            for epoch in tq:
                ...
                epoch_t0 = time.time()
                train_loss, metric_dict = self._train_epoch(epoch=epoch,
    ...
    def _synergy_optimizer(self, config):
        ...
        finetune_dict = optimparam_dict(finetune_p, config, lr=1e-6)
