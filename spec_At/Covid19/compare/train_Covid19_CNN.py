...
class Trainer(TrainerBase):
    ...
    def _train_epoch(self, epoch, data_loader, data_sampler, partition, inter_criterion=None):
        ...
        if inter_criterion is None:  inter_criterion = torch.nn.CrossEntropyLoss(weight=torch.FloatTensor([0.9, 0.1]).to(self.device))
        with context:
            y_true, y_prob = [], []
...
def train(model_ckpt=None, try_gpu=True, id_fold=None):
    ...
    data = GenDataloader(config, train_val_df, init_pos_data, input_seqtype='AbAtSeq', embed_type='CKSAAP')
