"""利用backbone的坐标得到邻接矩阵, 进而进行GCN和图嵌入"""
import os
import sys
...
from training.record import CSVWriter, clean_config, save_dict, Namespace
...
from spec_At.Covid19.databaseCovid import GenDataloader, get_nfold_datadf, load_batch_2device, save_nfold_df2csv, try_read_train_val
...
def init_config(config):
    ...
    config.par_out_dir = osp.join(spect_AtDir, f'trained_compare_models/coords_GCN_{config.embed_type}')
    ...
    def _train_epoch(self, epoch, data_loader, data_sampler, partition):
        ...
        with context:
            ...
            for batch in tqdm(data_loader):
                i_ += 1
                # if i_ < 916:  continue  # debug
                if batch.model_in is None:
                    continue
                else:
                    # elif self.num_data_workers > 0:
                ...
                if i_ % bn == 0:  # 模拟batch_size为32的情形
                    if partition == 'train':
                        self.optimizer.step()
                        self.optimizer.zero_grad()

                losses.update(loss_ls, no_batch=True, n=1)
                if batch.model_in.Ab_coords_label is not None:
                    prmsd_losses.update(prmsd_lossls, no_batch=True, n=1)
                    coords_losses.update(coords_lossls, no_batch=True, n=1)
                    bondlen_losses.update(bondlen_lossls, no_batch=True, n=1)
                inter_losses.update(inter_lossls, no_batch=True, n=1)
...
def train(model_ckpt=None, try_gpu=True, id_fold=None):
    ...
    # Covid_df['ab_dataid'] = range(Covid_df.shape[0])
    ...
    config.num_data_workers = data.num_data_workers
